...
if (PVS_STUDIO_AS_SCRIPT)
    ...
    if (result OR error)
        message(WARNING "${PVS_STUDIO_COMMAND}" "${additional_args}")
        message(${stderr_type} "${error}")
    endif ()

    foreach (COMMAND_ARG ${PVS_STUDIO_COMMAND})
        string(FIND "${COMMAND_ARG}" "plog" IN_PLOG)

        if (IN_PLOG GREATER "-1")
            file(RENAME ${COMMAND_ARG} ${COMMAND_ARG}.old)
            file(STRINGS ${COMMAND_ARG}.old PLOG_ITEMS)
            file(TOUCH ${COMMAND_ARG})

            foreach (PLOG_ITEM ${PLOG_ITEMS})
                string(FIND "${PLOG_ITEM}" "E:\\\\cloud_companion" SOURCE_PATH)

                if (SOURCE_PATH GREATER "-1")
                    string(FIND "${PLOG_ITEM}" "E:\\\\cloud_companion\\\\cmake-build-debug" BUILD_PATH)
                    if (BUILD_PATH EQUAL "-1")
                        file(APPEND ${COMMAND_ARG} "${PLOG_ITEM}\n")
                    endif()
                endif ()
            endforeach ()
        endif ()
...
function(pvs_studio_analyze_target TARGET DIR)
    ...
    if (NOT "${PROPERTY}" STREQUAL "NOTFOUND" AND NOT "${PROPERTY}" STREQUAL "PROPERTY-NOTFOUND")
        foreach (SOURCE ${PROPERTY})
            string(FIND "${SOURCE}" "${BINARY_DIR}" IN_BUILD)

            if (IN_BUILD STREQUAL -1)
    ...
    if (PVS_STUDIO_PLOGS OR PVS_STUDIO_COMPILE_COMMANDS)
        ...
        if (NOT "${PVS_STUDIO_FORMAT}" STREQUAL "" OR PVS_STUDIO_OUTPUT)
            ...
            list(APPEND COMMANDS
                    ...
                    COMMAND "${PVS_STUDIO_CONVERTER}" "${PVS_STUDIO_CONVERTER_ARGS}" -t "${PVS_STUDIO_FORMAT}" "${PVS_STUDIO_LOG}.pvs.raw" -o "${PVS_STUDIO_LOG}" -a "64:1,2,3\;GA:1,2,3\;OP:1,2,3\;CS:1,2,3\;MISRA:1,2,3\;AUTOSAR:1,2,3\;OWASP:1,2,3"
                    )
